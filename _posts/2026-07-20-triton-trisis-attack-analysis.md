...
Detection mostly comes down to watching things that traditionally get ignored on the OT side unusual traffic crossing between IT and OT networks, unexpected files or logins on the engineering workstation, and monitoring who's actually using the TriStation protocol. Mitigation is about tightening access to that workstation and the controllers themselves least privilege, application allow-listing so disguised files can't run, and restricting who/what can even reach the safety controllers in the first place, since the protocol itself had no authentication to rely on.

## What this taught me

not every vulnerability is a coded bug. Sometimes it's a missing feature entirely like TriStation having zero authentication built in. That's an important mindset shift for vulnerability assessment work generally: you're not just scanning for known CVEs, you're also asking "does this system even have basic security controls in place at all?"

Also security teams can't treat OT/ICS as someone else's problem just because it's traditionally been engineering's territory. TRITON specifically targeted the systems designed to prevent physical harm that's a fundamentally different stakes level than data theft, and it shows attackers are willing to go after that layer even though it's historically under-monitored.
